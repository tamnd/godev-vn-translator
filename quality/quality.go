...
type Finding struct {
	// Rule is the identifier, L01 to L20. It is stable, it is what a report
...
type Input struct {
	...
	// eleven of the twenty rules want them and parsing 480 files twenty
	// times over is twenty times the work for the same answer.
...
//
// Eighteen of the twenty do. They compare a sequence pulled out of the English
...
func run(in Input, rules []Rule) []Finding {
	...
	for _, rule := range rules {
		...
		// there. Running them anyway produces nineteen findings per missing
