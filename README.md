...
| L16 | kept terms | notice | glossary terms the table keeps in English, translated anyway |
| L17 | transport | refuse | a route's own error page, stored as if it were a translation |
| L18 | notice | refuse | the Go copyright and license header, translated |
| L19 | self links | refuse | a link whose text is its own target, where the English writes the url plain |
| L20 | front matter form | refuse | the bracket the front matter is written in, which the site reads two ways |
...
L16 is the other half of L10. L10 reports a glossary term left standing in English where the table says to translate it, and it skips the rows that say keep this term in English, so half the table was never checked. A page writing `thỏa mãn một giao diện` where the table says `interface` stays `interface` passed all fifteen gates that existed. It is a notice and not a refusal for the same reason L10 is: a term rendered some other way is often the page being read well.

L17 is the only rule here that is not about a translation. It is about a non-translation. `blog/go-slices-usage-and-internals.md` is a two line redirect stub, and a run left an OpenAI error page underneath it, because three of the four routes are a headless browser driving `chatgpt.com` and the scraper reads an error page as readily as an answer. Nothing downstream was broken: the piece was short, the answer was short, so L03 had nothing to call truncation against, and every other gate compares a translation with its English and has no reason to object to a sentence. The engine now refuses these at the door, where the right response is failing the route rather than the answer, and the rule covers the files that were written before the door existed.

L19 and L20 are both the same blind spot, found twice. Almost every gate above reads a sequence out of the English and asks whether the translation still has it, and a gate shaped that way cannot see anything the translation added or any change that keeps all the parts and alters what they mean. L19 is the added half: 202 of the 217 self links under `_content_vi` point at a target the English never links at all, so L07 had nothing to compare them against. L20 is the other half. Twelve pages came back with `<!--{ "Title": ... }-->` rewritten as `---` / `Title: ...` / `---`, and L09 passed all twelve because it compares the key names and the key names had not changed. Only the bracket around them had. The site reads the two forms in `internal/web/page.go` and lowercases the keys of the first and not the second, so `Template: true` in the wrong bracket is a page that has silently stopped being a template, and four of the twelve kept the object and wrapped fences round the outside, which hands `<!--{` to `yaml.Unmarshal` and stops the page rendering at all. `godev repair` puts the English's bracket back on every one it can, and L20 refuses what is left.
...
quality/     the twenty gates, the report, and translations.json
