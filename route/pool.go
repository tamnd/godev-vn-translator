...
// client builds the transport for a route, which is the one place the three
// kinds of route stop looking alike.
//
// A command route is a program on this machine and package codex runs it. A box
// and a gateway are both an address and package api calls it. Everything above
...
func (p *Pool) client(value Route) (api.Completer, error) {
	if value.IsCommand() {
		timeout := p.Timeout
		if value.Timeout > 0 {
			timeout = value.Timeout.Duration()
		}
		return &codex.Client{Timeout: timeout}, nil
	}
	return value.Client(p.Timeout, p.MaxRetries)
}
